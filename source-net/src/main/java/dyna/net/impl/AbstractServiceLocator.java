...
public abstract class AbstractServiceLocator implements ServiceLocator
{
	...
	private ServiceStateSync									ssSync				= null;

	private final Map<String, Map<Class<? extends Service>, Service>>	serviceCache	= Collections
	.synchronizedMap(new HashMap<String, Map<Class<? extends Service>, Service>>());

	private final Map<String, Class<? extends Service>>				serviceClassCache	= Collections
	.synchronizedMap(new HashMap<String, Class<? extends Service>>());

	public AbstractServiceLocator()
	{
		this.ssSync = new ServiceStateSync();
	}
	 ...
	 */
	@Override
	public ServiceBean getServiceBean(Class<? extends Service> serviceClass)
	{
	...
	protected <T extends Service> Class<T> getServiceClassById(String serviceId) throws ServiceNotFoundException
	{
		...
		{
			throw new ServiceNotFoundException("not found service: " + serviceBean.getServiceInterfaceName(),
					e.fillInStackTrace());
		}
	}
	...
	private Map<Class<? extends Service>, Service> getServiceMap(String credential)
	{
		...
		{
			this.serviceCache.put(credential,
					Collections.synchronizedMap(new HashMap<Class<? extends Service>, Service>()));
		}
	 ...
	 */
	@Override
	public List<ServiceBean> listServiceBean()
	{
