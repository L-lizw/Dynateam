...
public class IntegrateSM extends IntegrateERP<ERPSMOperationEnum>
{
			...
			throws ServiceRequestException
	{
		...
		String tableName = null;
		List<FoundationObject> substituteList = this.stub.getStubService().getBRM().listReplaceDataByRang(end1Obj.getObjectGuid(), end2Obj.getObjectGuid(), ReplaceRangeEnum.PART,
				ReplaceTypeEnum.TIDAI, bomTemplate.getName(), bomStructure.getBOMKey(), true);
	...
	private RecordSet getEachGlobalS4SM(FoundationObject end1Obj, BOMTemplateInfo bomTemplate, ERPSMOperationEnum operation) throws ServiceRequestException
	{
		...
		while (tableIt.hasNext())
		{
			tableName = tableIt.next();
			TableRecordData tableRecord = new TableRecordData();
			List<FoundationObject> substituteList = this.stub.getStubService().getBRM().listReplaceDataByRang(null, end1Obj.getObjectGuid(), ReplaceRangeEnum.GLOBAL,
					ReplaceTypeEnum.TIDAI, null, null, true);
			if (substituteList != null && substituteList.size() != 0)
			{
				for (int j = 0; j < substituteList.size(); j++)
				{
			...
			throws ServiceRequestException
	{
		...
		{
			tableName = tableIt.next();
			List<FoundationObject> substituteList = this.stub.getStubService().getBRM().listReplaceDataByRang(end1Obj.getObjectGuid(), end2Obj.getObjectGuid(),
					ReplaceRangeEnum.PART, ReplaceTypeEnum.QUDAI, bomTemplate.getName(), bomStructure.getBOMKey(), true);
			TableRecordData tableRecord = new TableRecordData();
			if (substituteList != null && substituteList.size() != 0)
			{
				for (int j = 0; j < substituteList.size(); j++)
			...
			if (object != null)
			{
				if (object instanceof BOMStructure)
				{
					...
					if (((BOMStructure) object).getEnd1ObjectGuid() != null && ((BOMStructure) object).getEnd2ObjectGuid() != null)
					{
						...
						list = brm.listReplaceDataByRang(((BOMStructure) object).getEnd1ObjectGuid(), ((BOMStructure) object).getEnd2ObjectGuid(), ReplaceRangeEnum.PART, type,
								bomTemplate.getName(), ((BOMStructure) object).getBOMKey(), true);
					}
				}
				else if (object instanceof FoundationObject)
				{
					list = brm.listReplaceDataByRang(null, object.getObjectGuid(), ReplaceRangeEnum.GLOBAL, ReplaceTypeEnum.TIDAI, null, null, true);
				}
				if (!SetUtils.isNullList(list))
				{
