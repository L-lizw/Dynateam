 ...
 * 
 */
public class AASImpl extends BusinessRuleService implements AAS
{
	/**
	 * 需要权限判断, 是否为操作者本人的方法
	 */
	 ...
	 * 需要权限判断, 是否为管理组内用户的方法
	 */
	private static final List<String>	ADMIN_METHOD_LIST	= Arrays.asList("logout", "revokeUserFromRoleInGroup", "revokeRoleFromGroup", "assignRoleToGroup",
			"assignUserToRoleInGroup", "saveUser", "saveGroup", "saveRole", "inactiveUser", "inactiveGroup", "inactiveRole", "activeUser", "activeGroup", "activeRole",
			"resetUserPassword");

	private GroupStub					groupStub			= null;
	private OrgStub						orgStub				= null;
	private RoleStub					roleStub			= null;
	private LoginStub					loginStub			= null;
	...
	public UserStub getUserStub()
	{
		...
		{
			this.userStub = new UserStub(this.serviceContext, this);
		}
	...
	public GroupStub getGroupStub()
	{
		...
		{
			this.groupStub = new GroupStub(this.serviceContext, this);
		}
	...
	{
		if (this.orgStub == null)
		{
			this.orgStub = new OrgStub(this.serviceContext, this);
		}
		return this.orgStub;
	}
	...
	public LoginStub getLoginStub()
	{
		...
		{
			this.loginStub = new LoginStub(this.serviceContext, this);
		}
	...
	@Tracked(description = TrackedDesc.LOGIN, renderer = TRLoginImpl.class)
	@Override
	...
	// * (non-Javadoc)
	// *
	// * @see dyna.net.service.brs.AAS#getServiceVersion()
	// */
	// @Override
	// public void getServiceVersion(String versionInfo) throws ServiceRequestException
	// {
	//
	// if (!Version.getVersionInfo().equalsIgnoreCase(versionInfo))
	// {
	// new ServiceRequestException("ID_APP_INVALID_CLIENT_VERSION", "Client Version inconsistencies", null,
	// Version.getVersionInfo(), versionInfo);
	// }
	// }
	 ...
	 */
	@Override
	public boolean checkClientVersion(String clientSum, String clientType, boolean isDebug) throws ServiceRequestException
	{
		if (this.serviceContext.getServerContext().isDebugMode() || isDebug)
		{
			if (this.getSignature() instanceof ModuleSignature)
			{
				((ModuleSignature) this.getSignature()).setVersionValidate(true);
			}
			return true;
		}
